...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from DU Github (http://github.com/KilluaHatake)." % device

repositories = []

page = 1
while not depsonly:
    request = Request("https://api.github.com/users/KilluaHatake/repos?page=%d" % page)
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'KilluaHatake/%s already exists' % (repo_name)
            else:
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for KilluaHatake/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: KilluaHatake/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "KilluaHatake/%s" % repo_name, "revision": "n7x-caf" })
